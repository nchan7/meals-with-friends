...
const Details: React.FC<IRestaurantProps> = ({api_id}) => {
  ...
  useEffect( () => {
    setRestaurant_id(api_id)
    console.log("Running the effect")
    axios.get(`/restaurants/${restaurant_id}`).then( (response) => {
  ...
  function handleReviewSubmit(api_id: number, name: String) {
    // e.preventDefault()
    // axios.post('/reviews', {
    //   search: search
    //   }).then(res => {
  ...
  if (restaurant !==null && Object.keys(restaurant).length > 0) {
      return (
        <div className='restaurantdetails'>
          ...
          <button>Add A Review</button> <br/> <br/>
        </div>
      )
  } else {
...
return (
    <>
      {restaurantDetails}
      <h3>Add a Review</h3>
      <form onSubmit={() => handleReviewSubmit(restaurant.id, restaurant.name)}>
